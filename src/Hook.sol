...
contract InternalSwapPool is BaseHook {
    ...
    constructor (address _poolManager, address _nativeToken) BaseHook(IPoolManager(_poolManager)) {
        // Ensure that our contract conforms to the expected address for Hook logic
     ...
     */
    function poolFees(PoolKey calldata _poolKey) public view override returns (ClaimableFees memory) {
     ...
     */
    function depositFees(PoolKey calldata _poolKey, uint _amount0, uint _amount1) public override {
        if (_amount0 != 0) {
    ...
    function _distributeFees(PoolKey calldata _poolKey) internal {
        ...
        donateAmount = (donateAmount > donateThresholdMax) ? donateThresholdMax : donateAmount;
        ...
        if (poolFee > 0) {
            ...
            (uint amount0, uint amount1) = (poolCurrencyFlipped[poolId]) ? (uint(0), poolFee) : (poolFee, uint(0));
     ...
     */
    function beforeSwap(address sender, PoolKey calldata key, IPoolManager.SwapParams memory params, bytes calldata hookData) public override poolManagerOnly returns (bytes4 selector_, BeforeSwapDelta beforeSwapDelta_, uint24 swapFee_) {
        ...
        bool trigger = poolCurrencyFlipped[poolId] ? !params.zeroForOne : params.zeroForOne;
        if (trigger && _poolFees[poolId].amount1 != 0) {
            ...
            if (poolCurrencyFlipped[poolId]) {
                poolManager.take(key.currency1, address(this), ethOut);
                _settleCurrency(key.currency0, tokenIn);
            } else {
                poolManager.take(key.currency0, address(this), ethOut);
                _settleCurrency(key.currency1, tokenIn);
            }
        }
     ...
     */
    function afterSwap(address sender, PoolKey calldata key, IPoolManager.SwapParams calldata params, BalanceDelta delta, bytes calldata hookData) public override poolManagerOnly returns (bytes4 selector_, int128 hookDeltaSpecified_) {
     ...
     */
    function beforeAddLiquidity(address sender, PoolKey calldata key, IPoolManager.ModifyLiquidityParams calldata params, bytes calldata hookData) public override poolManagerOnly returns (bytes4) {
     ...
     * @return selector_ The function selector for the hook
     */
    function beforeRemoveLiquidity(address sender, PoolKey calldata key, IPoolManager.ModifyLiquidityParams calldata params, bytes calldata hookData) public override poolManagerOnly returns (bytes4 selector_) {
        // Distribute fees to our LPs
        _distributeFees(key);
     ...
     * ..
     */
    function _settleCurrency(Currency _currency, uint _amount) internal {
        poolManager.sync(_currency);
        if (_currency.isNative()) {
            poolManager.settle{value: _amount}(currency);
        } else {
            IERC20Minimal(Currency.unwrap(_currency)).transfer(address(poolManager), _amount);
        }
        poolManager.settle(_currency);
    }
